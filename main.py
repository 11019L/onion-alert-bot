...
async def pay(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.effective_message.reply_text("Payment processing is manual. Contact admin.")

async def stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.effective_message.reply_text("Usage: `/stats yourusername`", parse_mode="Markdown")
        return
    influencer = ctx.args[0].lower()
    if influencer not in tracker:
        await update.effective_message.reply_text(f"No data for **{influencer}**.")
        return
...
async def owner(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("Dashboard error.")
...
def main():
    ...
    app.run_polling()  # ← FIXED: REMOVED drop_pending_updates
