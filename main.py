...
SAVE_INTERVAL = 30  # secondsdef load_data():
    ...
    with open(DATA_FILE, "w") as f:
        json.dump(tmp, f, indent=2)data = load_data()
...
goplus_cache = {}
save_lock = asyncio.Lock()# === AUTO-SAVE ===
async def auto_save():
    while True:
        await asyncio.sleep(SAVE_INTERVAL)
        async with save_lock:
            save_data(data)
            logger.info("Data saved.")# === HELPERS ===
def get_dex_url(chain: str, pair_addr: str) -> str:
    chain_slug = "solana" if chain == "SOL" else "bsc"
    return f"https://dexscreener.com/{chain_slug}/{pair_addr}"def safe_md(text):
    return escape_markdown(str(text), version=2)def format_alert(chain, sym, addr, liq, fdv, vol, pair_addr, reason):
    return (
...
async def is_safe_batch(addrs, chain, session):
    ...
    url = GOPLUS_API.format(chain_id=chain_id, addrs=addrs_str)now = time.time()
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    influencer = source.split("_", 1)[1] if "_" in source else Noneif influencer and influencer not in tracker:
...
await ctx.bot.send_message(uid, test, parse_mode="MarkdownV2", disable_web_page_preview=True)async def pay(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Payment processing is currently manual. Contact admin.")async def stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
    ...
    await update.message.reply_text(msg, parse_mode="MarkdownV2")# === SCANNER ===
async def scanner(app: Application):
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15)) as session:
        while True:
            try:
                now = time.time()
                candidates = []            # --- Fetch new pairs + trending ---
            ...
            await asyncio.sleep(60)# === MAIN ===
async def main():
    app = Application.builder().token(BOT_TOKEN).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("pay", pay))
    app.add_handler(CommandHandler("stats", stats))
    app.add_handler(CommandHandler("owner", owner))app.create_task(scanner(app))
app.create_task(auto_save())

logger.info("ONION ALERTS LIVE — SENDING CA FROM DEXSCREENER")
await app.run_polling(drop_pending_updates=True)if __name__ == "__main__":
    try:
        asyncio.run(main())
